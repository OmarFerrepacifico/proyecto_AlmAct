...
<table class="display nowrap table table-sm table-bordered table-hover table-responsive-sm" style="text-align: center; width:100%" id="mydatatable" >
	<thead >
		<tr>
			...
			<th>VENTA NETA ASIGNADA</th>
			<th>VENTA OBJETIVO AL DIA</th>
			<th>VENTA NETA REAL</th>
		...
		<?php 
            for ($i=0; $i < count($obj); $i++) { 
                ...
                for ($ii=0; $ii < count($clientes1); $ii++) {
					$porcentaje=number_format(($clientes1[$ii]->{'venta_neta_real'}/(($clientes1[$ii]->{'venta_neta_asignada'}/count($diastotal))*count($diastranscurridos)))*100,2);
					$porcentaje2=number_format(($clientes1[$ii]->{'utilidad_real'}/(($clientes1[$ii]->{'utilidad_asignada'}/count($diastotal))*count($diastranscurridos)))*100,2);
					if ($porcentaje<=89.99) {
						$color="#FF0000";
						$bono=0;
					}
					...
					$comision=($clientes1[$ii]->{'utilidad_clasificacion_a'}*$clientes1[$ii]->{'a'})+($clientes1[$ii]->{'utilidad_clasificacion_b'}*$clientes1[$ii]->{'b'})+($clientes1[$ii]->{'utilidad_clasificacion_c'}*$clientes1[$ii]->{'c'})+($clientes1[$ii]->{'venta_top_low'}*$clientes1[$ii]->{'toplow'})+($clientes1[$ii]->{'utilidad_articulo_nuevo'}*$clientes1[$ii]->{'an'})+($clientes1[$ii]->{'utilidad_cliente_nuevo'}*$clientes1[$ii]->{'cn'})+($clientes1[$ii]->{'utilidad_clasificacion'}*$clientes1[$ii]->{'x'});
		?>
                <tr>
                    <td style="text-align:left"><?php echo $clientes1[$ii]->{'colaborador'}; ?></td>
                    <td style="text-align:right"><?php echo "$".number_format($clientes1[$ii]->{'venta_neta_promedio_3'}, 2); ?></td>
                    <td style="text-align:right"><?php echo "$".number_format($clientes1[$ii]->{'venta_neta_asignada'},2) ?></td>
                    <td style="text-align:right"><?php echo "$".number_format(($clientes1[$ii]->{'venta_neta_asignada'}/count($diastotal))*count($diastranscurridos),2) ?></td>
                    <td style="text-align:right"><?php echo "$".number_format($clientes1[$ii]->{'venta_neta_real'},2) ?></td>
                    <td style="text-align:right;background-color:<?php echo $color ?>"><?php echo $porcentaje."%" ?></td>
                    <td style="text-align:right"><?php echo "$".number_format($clientes1[$ii]->{'utilidad_promedio_3'},2) ?></td>
                    <td style="text-align:right"><?php echo "$".number_format($clientes1[$ii]->{'utilidad_asignada'},2) ?></td>
                    <td style="text-align:right"><?php echo "$".number_format(($clientes1[$ii]->{'utilidad_asignada'}/count($diastotal))*count($diastranscurridos),2) ?></td>
                    ...
                    <td style="text-align:right"><?php echo "$".number_format($comision+$bono,2) ?></td>
                </tr>
        <?php   
                }
            } 
