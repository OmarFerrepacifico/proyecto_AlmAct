...
$obj = clientes([ "consulta" => "proyeccionproveedores","f_ini" => date('d/m/Y',strtotime($_GET['fechainicial'])),"f_fin" => date('d/m/Y',strtotime($_GET['fechafinal'])),"vendedor" => $vendedor,"sucursal"=>$sucursal,"mes"=>$_GET['mes']]);
$obj=json_decode($obj);
	...
	<tbody >
		<?php 
            for ($i=0; $i < count($obj); $i++) { 
                ...
                for ($ii=0; $ii < count($clientes1); $ii++) {
                    if($clientes1[$ii]->{'venta_promedio'}!=0){
                        $porcentaje=($clientes1[$ii]->{'venta_real_al_dia'}/((($clientes1[$ii]->{'venta_promedio'}/$_GET['mes'])/count($diastotal))*count($diastranscurridos)))*100;
                    }
					else{
                        $porcentaje=100;
                    }
                    if($clientes1[$ii]->{'utilidad_promedio'}!=0){
                        $porcentaje2=($clientes1[$ii]->{'utilidad_real_al_dia'}/((($clientes1[$ii]->{'utilidad_promedio'}/$_GET['mes'])/count($diastotal))*count($diastranscurridos)))*100;
                    }
					else{
                        $porcentaje2=100;
                    }
		...
		?>
                <tr>
                    ...
                    <td style="text-align:right;background-color:<?php echo $color ?>"><?php echo number_format($porcentaje,2)."%" ?></td>
                    ...
                    <td style="text-align:right;background-color:<?php echo $color2 ?>"><?php echo number_format($porcentaje2,2)."%" ?></td>
		...
		$('#mydatatable').DataTable({
            ...
            "footerCallback": function (row, data, start, end, display) {
				...
				// Update footer
						...
						$(api.column(8).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display((pageTotal6/pageTotal5)*100) + "%</b> ");
						$(api.column(9).footer()).html("Suma de total bombeo");
						$(api.column(9).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display((pageTotal3/<?php echo $obj1[0][0]->{'venta_neta'};?>)*100) + "%</b> ");
            }
        });
