		...
		<?php 
            for ($i=0; $i < count($obj); $i++) { 
                ...
                for ($ii=0; $ii < count($clientes1); $ii++) {
					$porcentaje1=number_format(($clientes1[$ii]->{'venta_real'}/(($clientes1[$ii]->{'venta'}/count($diastotal))*count($diastranscurridos)))*100,2);
					$porcentaje2=number_format(($clientes1[$ii]->{'utilidad_real'}/(($clientes1[$ii]->{'utilidad'}/count($diastotal))*count($diastranscurridos)))*100,2);
                    $porcentaje3=number_format(($clientes1[$ii]->{'venta_real_toplow'}/(($clientes1[$ii]->{'toplow'}/count($diastotal))*count($diastranscurridos)))*100,2);
                    $porcentaje4=number_format(($clientes1[$ii]->{'venta_real_pacific'}/(($clientes1[$ii]->{'objetivo_pacific'}/count($diastotal))*count($diastranscurridos)))*100,2);
                    for ($i=1; $i < 5; $i++) { 
                        if($i==1 || $i==2){
                            if(${"porcentaje" . $i}>100){
                                ${"b" . $i}=2500;
