...
$obj=json_decode($obj);*/
$obj = clientes([ "consulta" => $_GET['mes'],"f_ini" => date('d/m/Y',strtotime($_GET['fechainicial'])),"f_fin" => date('d/m/Y',strtotime($_GET['fechafinal'])),"vendedor" => $vendedor,"sucursal"=>$sucursal,"cliente"=>$_GET['cliente']]);
$obj=json_decode($obj);
?>
<style>
    .tablas{
...
<table class="display nowrap table table-sm table-bordered table-hover table-responsive-sm" style="text-align: center; width:100%" id="mydatatable1" >
	...
	<tbody >
		<?php 
            for ($i=0; $i < count($obj); $i++) { 
                ...
                for ($ii=0; $ii < count($clientes1); $ii++) {
                    if($clientes1[$ii]->{'tipo'}=='Devolución'){
                        $porcentaje=0.00;
                    }else{
		...
		?>
                <tr>
                    <td style="text-align:left"><?php echo $clientes1[$ii]->{'documento'}; ?></td>
                    <td style="text-align:left"><?php echo $clientes1[$ii]->{'tipo'}; ?></td>
