...
$obj = clientes([ "consulta" => "proyeccionarticulos","f_ini" => date('d/m/Y',strtotime($_GET['fechainicial'])),"f_fin" => date('d/m/Y',strtotime($_GET['fechafinal'])),"vendedor" => $vendedor,"sucursal"=>$sucursal,"mes"=>$_GET['mes']]);
$obj=json_decode($obj);
		...
		<?php 
            for ($i=0; $i < count($obj); $i++) { 
                ...
                for ($ii=0; $ii < count($clientes1); $ii++) {
					...
					$porcentaje3=number_format(($clientes1[$ii]->{'utilidad_real_al_dia'}/((($clientes1[$ii]->{'utilidad_promedio'}/$_GET['mes'])/count($diastotal))*count($diastranscurridos)))*100,2);
					if ($porcentaje<=89.99) {
						$color="#FF0000";
						$bono=0;
					}
					...
					if($clientes1[$ii]->{'dias_de_inventario'}!=0){
						
						$diasinventario=number_format($clientes1[$ii]->{'stock_disponible'}/($clientes1[$ii]->{'dias_de_inventario'}/count($diastranscurridos)),0);
					}else{
						$diasinventario=0;
					}
					if($clientes1[$ii]->{'venta_real_al_dia'}!=0){
						$margen=number_format(($clientes1[$ii]->{'utilidad_real_al_dia'}/$clientes1[$ii]->{'venta_real_al_dia'})*100,2);
					}else{
						$margen=0;
					}
		?>
                <tr>
					...
					<td style="text-align:right"><?php echo number_format($clientes1[$ii]->{'cantidad_promedio'}/$_GET['mes'], 2); ?></td>
                    <td style="text-align:right"><?php echo number_format(($clientes1[$ii]->{'cantidad_promedio'}/$_GET['mes']/count($diastotal))*count($diastranscurridos),2) ?></td>
                    <td style="text-align:right"><?php echo number_format($clientes1[$ii]->{'cantidad_vendida_al_dia'},2) ?></td>
                    <td style="text-align:right;background-color:<?php echo $color ?>"><?php echo $porcentaje."%" ?></td>
					<td style="text-align:right"><?php echo number_format($clientes1[$ii]->{'stock_disponible'}, 0); ?></td>
                    <td style="text-align:right"><?php echo $diasinventario ?></td>
		...
		$('#mydatatable').DataTable({
            ...
            "footerCallback": function (row, data, start, end, display) {
				...
				// Update footer
						...
						$(api.column(10).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display(pageTotal7) + "</b> ");
				// Total over all pages
				...
				// Update footer
						...
						$(api.column(11).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display(pageTotal8) + "</b> ");
				// Total over this page
				pageTotal9 = api
				...
				// Update footer
						...
						$(api.column(12).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display(pageTotal9) + "</b> ");
				// Total over this page
				pageTotal10 = api
				...
				// Update footer
						...
						$(api.column(14).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display(pageTotal10) + "</b> ");
				// Total over this page
				pageTotal11 = api
				...
				// Update footer
						...
						$(api.column(15).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display(pageTotal11) + "</b> ");
				...
				// Update footer
						...
						$(api.column(13).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display((pageTotal9/pageTotal8)*100) + "%</b> ");
						$(api.column(16).footer()).html("Suma de total bombeo");
						$(api.column(16).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display((pageTotal6/pageTotal3)*100) + "%</b> ");
						$(api.column(17).footer()).html("Suma de total bombeo");
						$(api.column(17).footer()).html(" <b>" + $.fn.dataTable.render.number(",", ".", 2, "").display((pageTotal3/<?php echo $obj1[0][0]->{'venta_neta'};?>)*100) + "%</b> ");
            }
        });
