...
$obj = clientes([ "consulta" => "proyeccion","f_ini" => date('d/m/Y',strtotime($_GET['fechainicial'])),"f_fin" => date('d/m/Y',strtotime($_GET['fechafinal'])),"vendedor" => $vendedor,"sucursal"=>$sucursal]);
$obj=json_decode($obj);
?>
<style>
    .tablas{
		...
		<?php 
            for ($i=0; $i < count($obj); $i++) { 
                ...
                for ($ii=0; $ii < count($clientes1); $ii++) {
					$porcentaje=number_format(($clientes1[$ii]->{'venta_neta'}/(($clientes1[$ii]->{'venta_neta_asignada'}/count($diastotal))*count($diastranscurridos)))*100,2);
					$porcentaje2=number_format(($clientes1[$ii]->{'venta_utilidad'}/(($clientes1[$ii]->{'utilidad_asignada'}/count($diastotal))*count($diastranscurridos)))*100,2);
					if ($porcentaje<=89.99) {
						$color="#FF0000";
						$bono=0;
					}
